...
public class DspMetadataApiController {
    ...
    public DspMetadataApiController(ParticipantContextService participantContextService, VersionProtocolService service, TypeTransformerRegistry transformerRegistry) {
    ...
    public Response getProtocolVersions(@PathParam("participantContextId") String participantContextId) {
        ...
        var versions = result.getContent().protocolVersions().stream()
                .map(protocolVersion -> new ProtocolVersion(protocolVersion.version(), "/" + participantContextId + protocolVersion.path(), protocolVersion.binding()))
                .toList();
