...
public class DcpScopeConfigurationExtensionTest {
    ...
    @Test
    void initialize(ServiceExtensionContext context, DcpScopeConfigurationExtension ext) {
        ...
        assertThat(captor.getValue().getId()).satisfies(scope -> {
            ...
            assertThat(captor.getValue().getType()).isEqualTo(DcpScope.DcpScopeType.DEFAULT.name());
        });
    }

    @Test
    void initialize_withPolicyType(ServiceExtensionContext context, DcpScopeConfigurationExtension ext) {
        var cfg = ConfigFactory.fromMap(Map.of(
                "membership.id", "membership-scope",
        ...
        assertThat(captor.getValue()).satisfies(scope -> {
            ...
            assertThat(scope.getType()).isEqualTo(DcpScope.DcpScopeType.POLICY.name());
